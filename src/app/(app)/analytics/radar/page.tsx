  ...
  return (
    <div className="min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)] p-6 md:p-10 font-sans">
      <div className="max-w-7xl mx-auto">
        <button onClick={() => router.push("/dashboard")} className="text-[var(--text-secondary)] hover:text-[var(--text-primary)] text-sm mb-4 inline-flex items-center gap-1 transition-colors">
          <ArrowLeft className="w-4 h-4" /> Kembali ke Dashboard
        </button>
        <h1 className="text-3xl md:text-4xl font-bold tracking-tight mb-2 text-slate-800">Analisis Kemampuan</h1>
        <p className="text-[var(--text-secondary)] mb-8">Detail performa per subtes dan perkembangan skor IRT-mu.</p>

        {hasRadarData && (
          <div className="bg-[var(--pastel-blue)]/30 border border-[var(--accent)]/10 rounded-3xl p-6 mb-8 flex gap-5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-[0_4px_20px_rgba(0,0,0,0.03)]">
            <h2 className="text-lg font-semibold mb-6 text-slate-800">Radar Kemampuan vs Target</h2>
        ...
        <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-[0_4px_20px_rgba(0,0,0,0.03)]">
          <h2 className="text-lg font-semibold mb-6 text-slate-800">Detail Per Subtes</h2>
          {hasRadarData ? (
                    ...
                    return (
                      <tr key={s.subject} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="py-3 px-2 text-center">
                          {s.score > 0 ? (
                            <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${statusColors[s.status]}`}>{statusLabels[s.status]}</span>
                          ) : (
