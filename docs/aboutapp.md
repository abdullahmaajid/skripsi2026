...
- **Zero-Friction Context Injection:** Saat siswa menjawab salah, sistem menginjeksi metadata soal (teks soal, opsi yang dipilih siswa, dan kunci jawaban asli) ke dalam *context state* AI. Meskipun data kunci jawaban dikirim ke frontend untuk evaluasi, di UI siswa kunci tersebut ditutupi menggunakan masking `???` agar siswa tidak melihatnya secara langsung. AI di *backend* menerima data utuh untuk memberikan sapaan (*auto-greeting*) dan respons yang sangat kontekstual tanpa siswa harus melakukan *copy-paste* soal.
