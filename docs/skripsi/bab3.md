...
Selain itu, sistem juga terintegrasi secara *server-side* dengan layanan **Groq API** sebagai penyedia *Large Language Model* (LLM) untuk memproses logika *Socratic Scaffolding*. Seluruh panggilan ke LLM dilakukan di sisi *backend* (API Route) sehingga *prompt* beserta kunci jawaban tidak pernah terekspos ke klien (*browser* pengguna).
