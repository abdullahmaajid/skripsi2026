...
- **Siswa** memiliki cabang pilihan awal:
  - **Opsi 1 (Opsional):** Bertanya di chat panel AI sebelum menebak jawaban $\rightarrow$ (**AI Tutor** merespons dengan memberikan petunjuk tipis/*hint*, lalu siswa kembali ke pilihan menebak).
  - **Opsi 2 (Wajib):** Memilih opsi A/B/C/D/E dan mengeklik tombol "Jawab" $\rightarrow$ (Melangkah ke proses pengecekan jawaban oleh **Sistem**).
- **Sistem** langsung mengecek kebenaran jawaban tersebut.
  - **Jika salah (Percobaan 1):** **Sistem** memunculkan peringatan kuning "Kesempatan Terakhir". **AI Tutor** otomatis mengirim chat (*hint*). **Siswa** memiliki cabang pilihan:
    - **Opsi 1:** Langsung menjawab ulang $\rightarrow$ (Kembali ke proses pengecekan jawaban oleh **Sistem**).
    - **Opsi 2:** Membalas chat **AI Tutor** untuk berdiskusi dulu $\rightarrow$ (Siswa berdiskusi dengan AI dan tetap berada di soal ini, sebelum akhirnya menebak ulang).
  - **Jika salah (Percobaan 2 / Habis):** **Sistem** mengubah status soal menjadi "Dilewati". **AI Tutor** otomatis mengirim chat evaluasi. **Siswa** memiliki cabang pilihan:
    - **Opsi 1:** Berdiskusi dengan **AI Tutor** untuk mencari tahu kesalahannya $\rightarrow$ (Siswa tertahan berdiskusi di soal ini).
    - **Opsi 2:** Langsung mengeklik "Lanjut" $\rightarrow$ (Sistem berpindah dan memuat lembar soal berikutnya).
  - **Jika benar:** **Sistem** memunculkan notifikasi "Benar!" dan tombol "Lihat Pembahasan AI". **AI Tutor** otomatis mengirim chat apresiasi. **Siswa** memiliki cabang pilihan:
    - **Opsi 1:** Mengobrol dengan **AI Tutor** $\rightarrow$ (Siswa tertahan berdiskusi di soal ini).
    - **Opsi 2:** Mengeklik tombol "Lihat Pembahasan AI" $\rightarrow$ (Siswa tertahan membaca pembahasan di soal ini).
    - **Opsi 3 (Wajib):** Mengeklik tombol "Lanjut" $\rightarrow$ (Sistem berpindah dan memuat lembar soal berikutnya).
...
- **Siswa** dihadapkan pada 3 percabangan aksi akhir:
  - **Opsi 1:** Mengeklik "Ulangi Latihan" $\rightarrow$ **(Konektor A)** mengarah ke *Alur Nomor 5*.
  - **Opsi 2:** Mengeklik "Lihat Pembahasan" $\rightarrow$ **(Konektor B)** mengarah ke *Alur Nomor 4*.
  - **Opsi 3:** Mengeklik "Pilih Subtes Lain" $\rightarrow$ **(Konektor C)** mengarah ke *Alur Nomor 6*.
...
- **Siswa** dihadapkan pada percabangan interaksi yang akan terus me-loop (berputar):
  - **Opsi 1:** Memilih nomor soal lain di navigasi $\rightarrow$ (Sistem memuat detail soal baru tersebut dan siswa bebas berinteraksi lagi).
  - **Opsi 2:** Mengeklik tombol "Tanya Pembahasan AI" $\rightarrow$ (**AI Tutor** memberikan penjabaran konsep dan siswa tetap di soal ini).
  - **Opsi 3:** Mengirim chat balasan bebas $\rightarrow$ (**AI Tutor** membalas secara interaktif dan siswa tetap di soal ini).
  - **Opsi 4:** Mengeklik "Keluar/Kembali" dari halaman pembahasan $\rightarrow$ (Menutup mode review dan mengakhiri alur ini).
- **Status Akhir Alur:** [Lingkaran Hitam Sempurna (Selesai)]
...
- **Siswa** membaca soal dan dihadapkan pada aksi navigasi:
  - **Opsi 1:** Memilih jawaban dan mengeklik "Selanjutnya" $\rightarrow$ (Sistem menyimpan jawaban dan berpindah ke soal berikutnya).
  - **Opsi 2:** Mengeklik "Sebelumnya" $\rightarrow$ (Sistem berpindah mundur ke soal sebelumnya).
  - **Opsi 3:** Mencentang kotak "Ragu-ragu" $\rightarrow$ (Sistem memberi tanda peringatan kuning pada nomor navigasi soal tersebut).
...
- **Siswa** dihadapkan pada 2 percabangan aksi akhir:
  - **Opsi 1:** Mengeklik "Lihat Review Jawaban" $\rightarrow$ **(Konektor A)** mengarah ke *Alur Nomor 9 (Mode Tampilan)*.
  - **Opsi 2:** Mengeklik "Bahas dengan AI Tutor" $\rightarrow$ **(Konektor B)** mengarah ke *Alur Nomor 9 (Mode Chat Socratic)*.
...
- **Siswa** dihadapkan pada 3 percabangan tab yang bisa dipindah kapan saja secara bebas:
  - **Opsi 1:** Tab "Rapor & Tren" $\rightarrow$ **(Konektor A)** mengarah ke *Alur Nomor 10B*.
  - **Opsi 2:** Tab "Evaluasi Soal" $\rightarrow$ **(Konektor B)** mengarah ke *Alur Nomor 11*.
  - **Opsi 3:** Tab "Peluang Lolos" $\rightarrow$ **(Konektor C)** mengarah ke *Alur Nomor 13*.
